...
def main(_):
    ...
    with tf.Graph().as_default():
        ...
        with tf.device(deploy_config.variables_device()):
            global_step = slim.create_global_step()     # <tf.Variable 'global_step:0' shape=() dtype=int64_ref>

        # Select the dataset.
        dataset = dataset_factory.get_dataset(
            FLAGS.dataset_name, FLAGS.dataset_split_name, FLAGS.dataset_dir)    # <tensorflow.contrib.slim.python.slim.data.dataset.Dataset object at 0x7f4f16369cc0>
        ...
        image_preprocessing_fn = preprocessing_factory.get_preprocessing(
            preprocess_name, is_training=True)
        tf_utils.print_configuration(FLAGS.__flags, ssd_params, dataset.data_sources, FLAGS.train_dir)

        # =================================================================== #
        # Create a dataset provider and batches.
        # =================================================================== #
        with tf.device(deploy_config.inputs_device()):  # /device:CPU:0
            ...
            [image, shape, glables, gbboxes] = provider.get(['image', 'shape', 'object/label', 'object/bbox'])
            print("image: %s\nshape: %s\nglables: %s\ngbboxes: %s\n" % (image, shape, glables, gbboxes))
            # Pre-processing image, labels and bboxes.
            image, glables, gbboxes = image_preprocessing_fn(image, glables, gbboxes,
                                                             out_shape=ssd_shape, data_format=DATA_FORMAT)
            # Encode groundtruth labels and bboxes.
            gclasses, glocalisations, gscores = ssd_net.bboxes_encode(glables, gbboxes, ssd_anchors)
            ...
            # Intermediate queueing: unique batch computation pipeline for all GPUs running the training.
